...
export function SolutionPreview({ plan, input, layers, flipLength, flipWidth, onDownloadReport }: SolutionPreviewProps) {
  ...
  const drawPalletView = useCallback(() => {
    ...
    // 绘制真实塑料托盘
    drawRealisticPallet(ctx, toIso, cx, cy, scale, palletL, palletW, palletH);
...
/** 绘制真实蓝色塑料托盘（3×3脚墩 + 网格顶面 + 横梁） */
function drawRealisticPallet(
  ctx: CanvasRenderingContext2D,
  toIso: (x: number, y: number, z: number) => [number, number],
  ...
  // 托盘颜色
  const blueTop = '#2563EB';     // 顶面蓝色
  const blueFront = '#1D4ED8';   // 正面蓝
  const blueRight = '#1E40AF';   // 右侧面蓝
  const blueFoot = '#1E3A8A';    // 脚墩深蓝
  const blueBeam = '#1E40AF';    // 横梁蓝
  const blueEdge = '#93C5FD';    // 顶面边缘高光

  // 结构比例
  const deckH = pH * 0.22;      // 顶板厚度
  const footH = pH - deckH;     // 脚墩高度
  const footL = pL * 0.08;      // 脚墩长方向尺寸
  const footW = pW * 0.08;      // 脚墩宽方向尺寸
  const beamH = footH * 0.35;   // 横梁高度

  // 脚墩位置 (3×3布局)
  const footPositions = [
    { x: pL * 0.1, y: pW * 0.1 },
    { x: pL * 0.5 - footL / 2, y: pW * 0.1 },
    { x: pL * 0.9 - footL, y: pW * 0.1 },
    { x: pL * 0.1, y: pW * 0.5 - footW / 2 },
    { x: pL * 0.5 - footL / 2, y: pW * 0.5 - footW / 2 },
    { x: pL * 0.9 - footL, y: pW * 0.5 - footW / 2 },
    { x: pL * 0.1, y: pW * 0.9 - footW },
    { x: pL * 0.5 - footL / 2, y: pW * 0.9 - footW },
    { x: pL * 0.9 - footL, y: pW * 0.9 - footW },
  ];

  // === 1. 先画脚墩（被顶板遮挡的部分） ===
  for (const fp of footPositions) {
    const [f1, f2, f3, f4, f5, f6, f7, f8] = [
      toIso(fp.x, fp.y, footH),
      toIso(fp.x + footL, fp.y, footH),
      toIso(fp.x + footL, fp.y + footW, footH),
      toIso(fp.x, fp.y + footW, footH),
      toIso(fp.x, fp.y, 0),
      toIso(fp.x + footL, fp.y, 0),
      toIso(fp.x + footL, fp.y + footW, 0),
      toIso(fp.x, fp.y + footW, 0),
    ];
    // 右侧面
    ctx.beginPath();
    ctx.moveTo(tx(f2[0]), ty(f2[1])); ctx.lineTo(tx(f3[0]), ty(f3[1]));
    ctx.lineTo(tx(f7[0]), ty(f7[1])); ctx.lineTo(tx(f6[0]), ty(f6[1]));
    ctx.closePath();
    ctx.fillStyle = blueRight; ctx.fill();
    ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.4; ctx.stroke();
    // 正面
    ctx.beginPath();
    ctx.moveTo(tx(f4[0]), ty(f4[1])); ctx.lineTo(tx(f3[0]), ty(f3[1]));
    ctx.lineTo(tx(f7[0]), ty(f7[1])); ctx.lineTo(tx(f8[0]), ty(f8[1]));
    ctx.closePath();
    ctx.fillStyle = blueFront; ctx.fill();
    ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.4; ctx.stroke();
    // 顶面
    ctx.beginPath();
    ctx.moveTo(tx(f1[0]), ty(f1[1])); ctx.lineTo(tx(f2[0]), ty(f2[1]));
    ctx.lineTo(tx(f3[0]), ty(f3[1])); ctx.lineTo(tx(f4[0]), ty(f4[1]));
    ctx.closePath();
    ctx.fillStyle = blueTop; ctx.fill();
    ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.3; ctx.stroke();
  }

  // === 2. 横梁连接脚墩 ===
  // 3条纵向横梁（沿长度方向）
  const beamRows = [pW * 0.1 + footW / 2, pW * 0.5, pW * 0.9 - footW / 2];
  for (const by of beamRows) {
    const bw = pW * 0.04;
    const z0 = 0;
    const z1 = beamH;
    // 正面
    const bp1 = toIso(0, by - bw / 2, z1);
    const bp2 = toIso(pL, by - bw / 2, z1);
    const bp3 = toIso(pL, by - bw / 2, z0);
    const bp4 = toIso(0, by - bw / 2, z0);
    ctx.beginPath();
    ctx.moveTo(tx(bp1[0]), ty(bp1[1])); ctx.lineTo(tx(bp2[0]), ty(bp2[1]));
    ctx.lineTo(tx(bp3[0]), ty(bp3[1])); ctx.lineTo(tx(bp4[0]), ty(bp4[1]));
    ctx.closePath();
    ctx.fillStyle = blueBeam; ctx.fill();
    ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.3; ctx.stroke();
    // 右侧面
    const rp1 = toIso(pL, by - bw / 2, z1);
    const rp2 = toIso(pL, by + bw / 2, z1);
    const rp3 = toIso(pL, by + bw / 2, z0);
    const rp4 = toIso(pL, by - bw / 2, z0);
    ctx.beginPath();
    ctx.moveTo(tx(rp1[0]), ty(rp1[1])); ctx.lineTo(tx(rp2[0]), ty(rp2[1]));
    ctx.lineTo(tx(rp3[0]), ty(rp3[1])); ctx.lineTo(tx(rp4[0]), ty(rp4[1]));
    ctx.closePath();
    ctx.fillStyle = blueRight; ctx.fill();
    ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.3; ctx.stroke();
    // 顶面
    const tp1 = toIso(0, by - bw / 2, z1);
    const tp2 = toIso(pL, by - bw / 2, z1);
    const tp3 = toIso(pL, by + bw / 2, z1);
    const tp4 = toIso(0, by + bw / 2, z1);
    ctx.beginPath();
    ctx.moveTo(tx(tp1[0]), ty(tp1[1])); ctx.lineTo(tx(tp2[0]), ty(tp2[1]));
    ctx.lineTo(tx(tp3[0]), ty(tp3[1])); ctx.lineTo(tx(tp4[0]), ty(tp4[1]));
    ctx.closePath();
    ctx.fillStyle = blueTop; ctx.fill();
    ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.3; ctx.stroke();
  }
  ...
  // === 3. 顶板（蓝色网格面板） ===
  // 顶面
  const d1 = toIso(0, 0, pH);
  const d2 = toIso(pL, 0, pH);
  const d3 = toIso(pL, pW, pH);
  const d4 = toIso(0, pW, pH);
  ctx.beginPath();
  ctx.moveTo(tx(d1[0]), ty(d1[1])); ctx.lineTo(tx(d2[0]), ty(d2[1]));
  ctx.lineTo(tx(d3[0]), ty(d3[1])); ctx.lineTo(tx(d4[0]), ty(d4[1]));
  ctx.closePath();
  ctx.fillStyle = blueTop; ctx.fill();
  ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.8; ctx.stroke();
  ...
  const gridCountL = 12;
  const gridCountW = 10;
  for (let i = 1; i < gridCountL; i++) {
    const gx = pL * i / gridCountL;
    const g1 = toIso(gx, 0, pH);
    const g2 = toIso(gx, pW, pH);
    ctx.beginPath(); ctx.moveTo(tx(g1[0]), ty(g1[1])); ctx.lineTo(tx(g2[0]), ty(g2[1])); ctx.stroke();
  }
  for (let j = 1; j < gridCountW; j++) {
    const gy = pW * j / gridCountW;
    const g1 = toIso(0, gy, pH);
    const g2 = toIso(pL, gy, pH);
    ctx.beginPath(); ctx.moveTo(tx(g1[0]), ty(g1[1])); ctx.lineTo(tx(g2[0]), ty(g2[1])); ctx.stroke();
  }
  ...
  // 顶板正面（边缘）
  const ef1 = toIso(0, pW, pH);
  const ef2 = toIso(pL, pW, pH);
  const ef3 = toIso(pL, pW, footH);
  const ef4 = toIso(0, pW, footH);
  ctx.beginPath();
  ctx.moveTo(tx(ef1[0]), ty(ef1[1])); ctx.lineTo(tx(ef2[0]), ty(ef2[1]));
  ctx.lineTo(tx(ef3[0]), ty(ef3[1])); ctx.lineTo(tx(ef4[0]), ty(ef4[1]));
  ctx.closePath();
  ctx.fillStyle = blueFront; ctx.fill();
  ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.5; ctx.stroke();

  // 顶板右侧面（边缘）
  const er1 = toIso(pL, 0, pH);
  const er2 = toIso(pL, pW, pH);
  const er3 = toIso(pL, pW, footH);
  const er4 = toIso(pL, 0, footH);
  ctx.beginPath();
  ctx.moveTo(tx(er1[0]), ty(er1[1])); ctx.lineTo(tx(er2[0]), ty(er2[1]));
  ctx.lineTo(tx(er3[0]), ty(er3[1])); ctx.lineTo(tx(er4[0]), ty(er4[1]));
  ctx.closePath();
  ctx.fillStyle = blueRight; ctx.fill();
  ctx.strokeStyle = '#1E3A8A'; ctx.lineWidth = 0.5; ctx.stroke();
}
